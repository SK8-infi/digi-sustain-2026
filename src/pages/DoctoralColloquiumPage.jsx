import { conferenceInfo, doctoralColloquiumHighlights } from '../data/conferenceData';
import SectionContainer, { SectionHeader } from '../components/ui/SectionContainer';
import Card, { CardTitle, CardDescription } from '../components/ui/Card';

export default function DoctoralColloquiumPage() {
    return (
        <>
            {/* Page Header */}
            <div style={{ backgroundColor: '#1a4731' }} className="py-16">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <h1 style={{ color: '#ffffff' }} className="text-4xl md:text-5xl font-bold mb-4">Doctoral Colloquium</h1>
                    <p style={{ color: '#9ca3af' }} className="text-xl max-w-3xl">
                        A dedicated platform for doctoral scholars to present their research, receive feedback, and network with senior academics.
                    </p>
                    <div style={{ backgroundColor: 'rgba(255,255,255,0.1)' }} className="mt-6 inline-flex items-center gap-2 backdrop-blur-sm px-4 py-2 rounded-lg">
                        <svg style={{ color: '#9ca3af' }} className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                        </svg>
                        <span style={{ color: '#ffffff' }} className="font-semibold">{conferenceInfo.doctoralColloquiumDate}</span>
                    </div>
                </div>
            </div>

            {/* Key Highlights */}
            <SectionContainer background="white">
                <SectionHeader
                    title="Key Highlights"
                    subtitle="What to expect at the Doctoral Colloquium"
                />
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {doctoralColloquiumHighlights.map((highlight, index) => (
                        <HighlightCard key={index} highlight={highlight} />
                    ))}
                </div>
            </SectionContainer>

            {/* Why Attend */}
            <SectionContainer background="light">
                <SectionHeader
                    title="Why Attend?"
                    subtitle="Benefits for doctoral scholars and early-career researchers"
                    centered={false}
                />
                <div className="grid md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                        <BenefitItem
                            title="Expert Feedback"
                            description="Receive constructive feedback on your research from established academics and industry experts."
                        />
                        <BenefitItem
                            title="Publication Opportunities"
                            description="Selected papers may be considered for publication in partner journals."
                        />
                        <BenefitItem
                            title="Methodology Training"
                            description="Attend workshops on quantitative and qualitative research methods."
                        />
                    </div>
                    <div className="space-y-4">
                        <BenefitItem
                            title="Networking"
                            description="Connect with fellow researchers, potential collaborators, and journal editors."
                        />
                        <BenefitItem
                            title="Recognition"
                            description="Compete for the Best Paper Award and gain visibility for your research."
                        />
                        <BenefitItem
                            title="Career Development"
                            description="Learn about publishing strategies and academic career paths from experienced scholars."
                        />
                    </div>
                </div>
            </SectionContainer>
        </>
    );
}

function HighlightCard({ highlight }) {
    const icons = {
        award: (
            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" />
            </svg>
        ),
        presentation: (
            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z" />
            </svg>
        ),
        workshop: (
            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
            </svg>
        ),
        writing: (
            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
            </svg>
        ),
        editors: (
            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
            </svg>
        ),
        networking: (
            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
            </svg>
        ),
    };

    return (
        <Card className="text-center">
            <div className="w-16 h-16 bg-primary-100 rounded-full flex items-center justify-center mx-auto mb-4 text-primary-700">
                {icons[highlight.icon] || icons.award}
            </div>
            <CardTitle>{highlight.title}</CardTitle>
            <CardDescription>{highlight.description}</CardDescription>
        </Card>
    );
}

function BenefitItem({ title, description }) {
    return (
        <div className="flex gap-4">
            <div className="w-10 h-10 bg-primary-100 rounded-lg flex items-center justify-center shrink-0">
                <svg className="w-5 h-5 text-primary-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                </svg>
            </div>
            <div>
                <h3 className="font-semibold text-neutral-900">{title}</h3>
                <p className="text-neutral-600 text-sm">{description}</p>
            </div>
        </div>
    );
}
